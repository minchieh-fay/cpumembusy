...
2. **资源波动算法**：
   ...
   - **期望占用值计算**：
     - 凌晨时段（UTC 16:00-20:00）：期望占用 = min(用户设置值, 80%)
     - 其他时段：期望占用 = min(用户设置值 / 2, 80%)
   - **内存控制算法**：
...
4. **峰值控制**：
   ...
   - 默认值：40%
   - **硬峰值限制**：80%（任何时段都不能超过 80%，保证其他应用的正常使用）
   - **示例**：
     - 用户设置 `P=70`：
       - 凌晨时段：可到 70%
       - 其他时段：最多到 35%（70% 的一半）
...
- 程序应记录关键操作日志，便于排查问题
  ...
  - **日志内容**：
    ...
    2. **每次随机调整时**：打印 rand 结果，明确说明是**增加资源**还是**减少资源**，以及调整的具体数值（内存调整量或 CPU sleep 值变化）
    3. **错误信息**：系统资源监控失败、内存分配失败等错误情况
